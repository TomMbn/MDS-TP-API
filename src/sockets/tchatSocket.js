      ...
      if (msg.startsWith("/dab ")) {
        ...
        if (isNaN(amount) || amount <= 0) {
          io.emit("chat message", {
            username: "Système",
            text: `Veuillez entrer un montant valide après /dab (ex: /dab 150 ou /dab 13.37)`,
          });
        } else {
          ...
          if (distribution.length === 0) {
            io.emit("chat message", {
              username: "Système",
              text: `Montant: ${amount}€, aucune coupure/pièce trouvée`,
            });
          } else {
            io.emit("chat message", {
              username: "Système",
              text: `Pour ${amount}€, vous avez : ${distribMessage}`,
            });
          }
        }
      } else {
